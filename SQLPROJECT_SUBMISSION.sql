CREATE DATABASE DBMS_TEAM3FINAL
GO
USE DBMS_TEAM3FINAL
GO
...
SELECT f.AircraftName, AVG(f.BaseFare)
FROM Flight f
JOIN Airline a
...
---------
INSERT INTO Bookings (BookingID,FareQuotes,CalculatedFare,ServiceFees,ServiceTax,UserID,BookingDate) VALUES (1,100,110,5,5,1,'07/23/2017');

--HotelBooking
select * from HotelBooking;

INSERT INTO HotelBooking (BookingID,FareRuleCode,HotelCode,BookingStartDate,BookingEndDate,BookingAdults,BookingRoomCount) VALUES ('1','1','1','07/21/2017','07/23/2017','1','1');
...
INSERT INTO DebitCardPayment (PaymentID,CardNumber,CardExpiry,CardPin,CardCVV,CardZip) VALUES ('1','5019008719012976','09/30/2018','1234','675','2115');
...
INSERT INTO Flight_Class  VALUES (1,'1');
INSERT INTO Flight_Class  VALUES (1,'2');
INSERT INTO Flight_Class  VALUES (1,'3');
INSERT INTO Flight_Class  VALUES (2,'1');
INSERT INTO Flight_Class  VALUES (2,'2');
INSERT INTO Flight_Class  VALUES (2,'3');
INSERT INTO Flight_Class  VALUES (3,'1');
INSERT INTO Flight_Class  VALUES (3,'2');
INSERT INTO Flight_Class  VALUES (3,'3');
INSERT INTO Flight_Class  VALUES (4,'1');
INSERT INTO Flight_Class  VALUES (4,'2');
INSERT INTO Flight_Class  VALUES (4,'3');
INSERT INTO Flight_Class  VALUES (5,'1');
INSERT INTO Flight_Class  VALUES (5,'2');
INSERT INTO Flight_Class  VALUES (5,'3');
INSERT INTO Flight_Class  VALUES (6,'1');
INSERT INTO Flight_Class  VALUES (6,'2');
INSERT INTO Flight_Class  VALUES (6,'3');
INSERT INTO Flight_Class  VALUES (7,'1');
INSERT INTO Flight_Class  VALUES (7,'2');
INSERT INTO Flight_Class  VALUES (7,'3');
INSERT INTO Flight_Class  VALUES (8,'1');
INSERT INTO Flight_Class  VALUES (8,'2');
INSERT INTO Flight_Class  VALUES (8,'3');
INSERT INTO Flight_Class  VALUES (9,'1');
INSERT INTO Flight_Class  VALUES (9,'2');
INSERT INTO Flight_Class  VALUES (9,'3');
INSERT INTO Flight_Class  VALUES (10,'1');
INSERT INTO Flight_Class  VALUES (10,'2');
INSERT INTO Flight_Class  VALUES (10,'3');
...
INSERT Bookings VALUES (2,200,15,10,2,'07/22/2017');
...
INSERT INTO BusBooking  VALUES (7,'8',2,'08/20/2017');
INSERT INTO BusBooking  VALUES (8,'7',4,'09/10/2017');
INSERT INTO BusBooking  VALUES (9,'6',5,'08/21/2017');
INSERT INTO FlightBooking  VALUES (10,'9',2,1);
INSERT INTO FlightBooking  VALUES (11,'10',4,2);
INSERT INTO FlightBooking  VALUES (12,'9',5,4);

--Triggers

--Compute Hotel Fare
CREATE TRIGGER COMPUTE_FARE
ON HotelBooking
after INSERT
AS
  BEGIN

	DECLARE @Discountval INTEGER;
	SET @Discountval =0;

	DECLARE @BaseFareVal MONEY;
	SET @BaseFareVal =0;


	DECLARE @FareQuoteVal MONEY;
	SET @FareQuoteVal = 0;
     
	 DECLARE @BookingIdVal INTEGER;
	SET @BookingIdVal = 0;

		SELECT @Discountval = fr.Discount
        FROM HotelBooking hb
		JOIN FareRule fr
		ON hb.FareRuleCode = fr.FareRuleCode;


		SELECT @BaseFareVal = h.BaseFare from Hotel h
		JOIN HotelBooking hb
		ON h.HotelCode = hb.HotelCode;


		SELECT @FareQuoteVal = bs.FareQuote,
		@BookingIdVal = hb.BookingID
		 from dbo.Bookings bs
		JOIN HotelBooking hb
		ON hb.BookingID = bs.BookingID;
		

		 UPDATE Bookings
		 SET    CalculatedFare = @BaseFareVal - (@Discountval/100) + (@FareQuoteVal/100)
		 WHERE BookingID = @BookingIdVal;
...
CREATE PROCEDURE SEARCHQUERY 
	@SearchParam VARCHAR(10),
	@StartDate DATETIME,
	@EndDate DATETIME
AS
BEGIN
	
	IF @SearchParam = 'Hotel'
		BEGIN
			select hl.HotelName,hl.ServiceCode,h.BaseFare,h.HotelStarRating from HotelListings hl
			JOIN Hotel h
			ON h.HotelCode = hl.HotelCode
			WHERE @StartDate BETWEEN hl.AvailabilityStartDate AND hl.AvailabilityEndDate
			AND @EndDate BETWEEN hl.AvailabilityStartDate AND hl.AvailabilityEndDate;
		END 
	IF @SearchParam = 'Car'
		BEGIN
			select c.CareModel,c.CarName,c.BaseFare from CarRentalListings crl
			JOIN Car c
			ON c.CarCode = crl.CarCode
			WHERE @StartDate BETWEEN crl.AvailabilityStartDate AND crl.AvailabilityEndDate;
		END
	IF @SearchParam = 'Flight'
		BEGIN
			select  f.AircraftName,f.FlightDuration,f.BaseFare,
			arp.AirportName as [Departure Airport],
			fsl.DepartureDate as [Departure Date] 
			from FlightScheduleListings fsl
			JOIN Flight f
			on f.FlightCode = fsl.FlightCode
			JOIN Airline a
			ON a.AirlineCode = f.AirlineCode
			JOIN Airport arp
			ON arp.AirportCode = f.SourceAirportCode
			WHERE @StartDate BETWEEN fsl.DepartureDate AND fsl.ArrivalDate ;
	...
	IF @SearchParam = 'Bus'
		BEGIN
			select bbl.BusOperator,bbl.JourneyDate,bbl.JourneyDuration,b.Name, b.Description 
			from BusBookingListings bbl
			JOIN Bus b
			ON b.BusCode = bbl.BusCode
			WHERE @StartDate=  bbl.JourneyDate;
...
DECLARE @SearchParam VARCHAR(10); 
DECLARE	@StartDate DATETIME;
DECLARE	@EndDate DATETIME;
-- Initialize variable 
SET @SearchParam = 'Car';
SET @StartDate = '12-08-2017';
SET @EndDate = '12-20-2017';
-- Execute the procedure
EXEC SEARCHQUERY @SearchParam, @StartDate,@EndDate;
...
-- Declare variables for BUS
DECLARE @SearchParam VARCHAR(10); 
DECLARE	@StartDate DATETIME;
DECLARE	@EndDate DATETIME;
-- Initialize variable 
SET @SearchParam = 'Bus';
SET @StartDate = '09-12-2017';
SET @EndDate = '12-20-2017';
-- Execute the procedure
EXEC SEARCHQUERY @SearchParam, @StartDate,@EndDate;

-- Declare variables for FLIGHT
DECLARE @SearchParam VARCHAR(10); 
DECLARE	@StartDate DATETIME;
DECLARE	@EndDate DATETIME;
-- Initialize variable 
SET @SearchParam = 'Flight';
SET @StartDate = '08-2-2017';
SET @EndDate = '12-20-2017';
-- Execute the procedure
EXEC SEARCHQUERY @SearchParam, @StartDate,@EndDate;
...
ALTER TABLE Bookings ADD CONSTRAINT RestrictOldDate CHECK (dbo.CheckOldDate(BookingDate) <= 0);


CREATE FUNCTION CheckOldDate (@BookDate date)
RETURNS smallint
AS
BEGIN
   DECLARE @Count smallint=0;
   set  @Count =datediff(day,@BookDate,getdate());
   RETURN @Count;
END;
...
INSERT INTO Bookings  VALUES (3,300,15,10,2,'09/01/2017');
INSERT INTO Bookings  VALUES (4,150,5,5,3,'10/09/2017');
INSERT INTO Bookings  VALUES (5,500,15,15,4,'11/22/2017');
INSERT INTO Bookings  VALUES (6,225,2,22,5,'10/25/2017');
INSERT INTO Bookings  VALUES (7,100,12,2,7,'09/24/2017');
INSERT INTO Bookings  VALUES (8,150,10,1,10,'09/28/2017');
INSERT INTO Bookings  VALUES (9,350,10,5,3,'10/22/2017');
INSERT INTO Bookings  VALUES (10,400,10,10,2,'12/25/2017');
INSERT INTO Bookings  VALUES (11,111,1,1,6,'11/22/2017');
INSERT INTO Bookings  VALUES (12,'450','10','5','8','01/01/2018');
...
INSERT INTO Payment  VALUES (1,'CardPayment','DebitCardPayment',110,'07/02/2017',1);
...
ALTER TABLE USERACCOUNT
ADD TotalBonusPointsEarned INTEGER;
...
SELECT t.TicketID, t.BookingID, p.Name, p.DOB, p.Gender, f.AirlineCode, f.AircraftName, 
...
ON l.CountryCode = cn.CountryCode
JOIN Class cl
ON f.FlightCode = cl.FlightCode
JOIN Seat st
ON cl.ClassCode = st.ClassCode
...
CREATE FUNCTION [dbo].[CheckCityExists](@Cname varchar(20))
RETURNS bit
AS
BEGIN
	DECLARE @Count bit=0;
	IF EXISTS (SELECT * FROM dbo.City WHERE CityName = @Cname)
		SET @Count = 1;
		RETURN @Count;
END;
...
CREATE FUNCTION [dbo].[CheckStateExists](@Sname varchar(20))
RETURNS bit
AS
BEGIN
	DECLARE @Count bit=0;
	IF EXISTS (SELECT * FROM dbo.State WHERE StateName = @Sname)
		SET @Count = 1;
		RETURN @Count;
END;
